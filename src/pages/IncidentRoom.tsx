...
import { 
  ...
  FileText, History, Layout, AlertTriangle, XCircle, Volume2, Pencil, Save
} from "lucide-react";
...
export default function IncidentRoom() {
  ...
  // Dramatic Conflict Flash State
  const [activeConflictFlash, setActiveConflictFlash] = useState<any>(null);
  const prevConflictsLengthRef = useRef(incident?.conflicts?.length || 0);

  useEffect(() => {
    const currentLen = incident?.conflicts?.length || 0;
    if (currentLen > prevConflictsLengthRef.current) {
      // Trigger flash for 3 seconds
      setActiveConflictFlash(incident.conflicts[0] || true);
      setTimeout(() => setActiveConflictFlash(null), 3000);
    }
    prevConflictsLengthRef.current = currentLen;
  }, [incident?.conflicts]);
  ...
  return (
    <div className="h-screen bg-bg-primary dark:bg-bg-primary text-text-primary dark:text-text-primary flex flex-col overflow-hidden relative transition-colors duration-200">
      ...
      {/* Dramatic Conflict Flash Overlay */}
      <AnimatePresence>
        {activeConflictFlash && (
          <motion.div
            ...
            className="absolute inset-0 z-[100] pointer-events-none flex items-center justify-center bg-state-conflict/20 backdrop-blur-sm"
          >
            <div className="absolute inset-0 bg-gradient-to-b from-state-conflict/40 via-transparent to-state-conflict/40 animate-pulse-slow" />
            <div className="bg-bg-surface border-2 border-state-conflict shadow-[0_0_100px_rgba(239,68,68,0.6)] px-12 py-8 rounded-3xl flex flex-col items-center text-center max-w-2xl transform">
              <TriangleAlert className="w-24 h-24 text-state-conflict mb-4 animate-[bounce_1s_infinite]" />
              <h2 className="text-4xl font-display font-bold text-state-conflict tracking-tighter uppercase">Conflict Detected</h2>
              <p className="text-text-primary mt-4 text-lg">
                {typeof activeConflictFlash === 'object' ? activeConflictFlash.description : 'Contradictory information detected in the room.'}
              </p>
            </div>
          </motion.div>
        )}
      ...
      <header className="relative z-10 h-16 border-b border-border-subtle flex items-center justify-between px-6 hud-panel shrink-0">
        ...
        <div className="flex items-center">
          ...
          <div className="h-4 w-px bg-zinc-300 dark:bg-bg-surface/10 mx-2" />
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-indigo-500/10 text-accent border border-border-subtle uppercase tracking-wider shadow-[0_0_10px_rgba(79,70,229,0.2)]">
                {incident.roomCode}
              </span>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col relative z-10">
          <div className="flex items-center px-6 h-12 border-b border-border-subtle dark:border-border-subtle gap-6">
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-bg-primary dark:bg-bg-primary">
            <AnimatePresence mode="wait">
              {activeTab === "overview" && (
                ...
                >
                  ...
                  <div className="glass-panel p-6 flex flex-col h-[calc(100vh-14rem)] rounded-3xl border border-border-subtle dark:border-border-subtle relative overflow-hidden bg-bg-surface dark:bg-bg-surface/40">
                    ...
                    <div className="mb-6 relative z-10 flex items-center justify-between">
                      <div>
                        <h3 className="font-bold text-lg text-text-primary dark:text-text-primary flex items-center gap-2">
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 space-y-4 relative z-10 flex flex-col">
                      ...
                      {aiPredictionFeed.map((item: any, idx: number) => {
                        ...
                        return (
                          <div key={item.id || idx} className={cn("rounded-2xl p-4 shadow-sm relative group border", meta.bgClass)} style={{ borderColor: `${meta.color}30` }}>
                            ...
                            <p className="text-[13px] font-semibold leading-relaxed pl-3 pr-1 text-text-primary dark:text-zinc-100">{item.text}</p>
                            <p className="text-[10px] text-text-muted mt-2 font-medium tracking-wide uppercase pl-3">{item.speaker}</p>
                          </div>
                        );
                      })}
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {incident.actions?.map((a: any) => (
                          <div key={a.id} className={cn(
                            "group p-4 bg-bg-surface/80 dark:bg-bg-surface/40 border border-border-subtle dark:border-border-subtle rounded-2xl hover:bg-bg-primary dark:hover:bg-zinc-900/60 transition-colors",
                            a.isCritical
                            ...
                            <div className="flex items-center gap-3 relative z-10">
                              <div className={cn(
                                ...
                                a.isCritical ? "bg-amber-500/20" : "bg-bg-surface dark:bg-bg-surface"
                              )}>
                                {a.isCritical
                                  ? <AlertTriangle className="w-4 h-4 text-state-hypothesis" />
                                  : <Zap className="w-4 h-4 text-accent" />}
                              </div>
                              <div>
                                <p className={cn("text-xs font-bold mb-0.5", a.status === 'DONE' && "line-through opacity-50")}>{a.description}</p>
                                <div className="flex items-center gap-2">
                                  <span className="text-[9px] text-text-muted uppercase tracking-wider">{a.owner?.name || "Unassigned"}</span>
                                  <span className={cn(
                                    "text-[9px] px-1.5 py-0.5 rounded border uppercase tracking-widest",
                                    a.status === 'DONE' ? "bg-emerald-500/10 text-state-fact border-emerald-500/20" :
                                    a.isCritical
                                      ? "bg-amber-500/10 text-state-hypothesis border-amber-500/20"
                                      : "bg-blue-500/10 text-accent border-blue-500/10"
                        ...
                        {(!incident.actions || incident.actions.length === 0) && (
                          <p className="text-text-muted text-xs italic text-center py-4">No action items assigned yet.</p>
                        )}
                      </div>
                    </Section>
                  </div>
        ...
        <aside className="w-80 border-l border-border-subtle dark:border-border-subtle flex flex-col hud-panel shrink-0 relative z-10 bg-bg-surface/50 dark:bg-bg-primary">
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    isAI ? "text-purple-100 bg-purple-500/10 border border-purple-500/20 rounded-2xl rounded-tl-sm" 
                         ...
                         : "text-zinc-800 dark:text-zinc-200 bg-bg-surface dark:bg-bg-surface rounded-2xl rounded-tl-sm border border-border-subtle dark:border-border-subtle"
                  )}>
                    {t.text}
                  </div>
                </motion.div>
          ...
          <div className="p-4 bg-bg-surface dark:bg-bg-primary border-t border-border-subtle dark:border-border-subtle">
            <form onSubmit={handleSendChat} className="flex gap-2">
              <input
                ...
                className="flex-1 bg-bg-surface dark:bg-bg-surface border border-border-subtle dark:border-border-subtle rounded-xl px-4 py-3 text-[13px] focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 text-text-primary dark:text-text-primary placeholder:text-text-muted dark:placeholder:text-text-muted transition-all shadow-inner"
              />
              <button
                type="submit"
...
function StatusItem({ icon, label, value }: { icon: React.ReactNode, label: string, value: string }) {
  return (
    <div className="flex items-center justify-between p-3 bg-bg-primary dark:bg-bg-surface/[0.02] border border-border-subtle dark:border-border-subtle rounded-2xl group hover:border-border-glow dark:hover:border-border-glow hover:shadow-[0_0_15px_rgba(79,70,229,0.1)] transition-all">
      <div className="flex items-center gap-2">
        {icon}
        <span className="text-[10px] font-bold text-text-muted dark:text-text-muted uppercase tracking-widest">{label}</span>
      </div>
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? "text-text-primary dark:text-text-primary border-blue-600 dark:border-blue-500" : "text-text-muted border-transparent hover:text-zinc-800 dark:hover:text-zinc-300"
      )}
    >
      {icon}
      {label}
